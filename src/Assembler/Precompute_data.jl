...
function compute_jacobian(elem_connect, nodes, dN_parametric)
    ...
    invJ_detJ = Vector{Tuple{Float64, Matrix{Float64}}}(undef, num_gauss)
    ...
    for e in 1:num_gauss
        ...
        J = if dim == 1
            J = sum(nodes[elem_connect[i], 1] * dN_ξ[i] for i in 1:n_nodes)
            J = reshape(J, 1, 1)  # Make it a matrix
        elseif dim == 2
        ...
        detJ = det(J)
        abs(detJ) < 1e-12 && error("Jacobian nearly singular at Gauss point $e")
...
function build_B_matrices(nodes::Matrix{Float64},
    ...
    Threads.@threads for e in 1:n_elem
        ...
        for B_type in material.B_types
            ...
            elseif B_type ∈ (:strain_rate, :gradient, :electric_field, :pressure, :temperature_gradient, :fluid_velocity)
